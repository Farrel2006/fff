...
# =====================================
    ...
    # 11. PROSES ISOBARIK (Tekanan Tetap)
        ...
        target = st.selectbox("Pilih variabel gas yang dicari:", ["V1 (Volume Awal)", "T1 (Suhu Awal)", "V2 (Volume Akhir)", "T2 (Suhu Akhir)"])
        
        V1 = st.number_input("V1 (L)", value=0.0) if target != "V1 (Volume Awal)" else 0.0
        T1 = st.number_input("T1 (K)", value=0.0) if target != "T1 (Suhu Awal)" else 0.0
        V2 = st.number_input("V2 (L)", value=0.0) if target != "V2 (Volume Akhir)" else 0.0
        T2 = st.number_input("T2 (K)", value=0.0) if target != "T2 (Suhu Akhir)" else 0.0

        if st.button("Hitung"):
            st.balloons()
            if "V1" in target:
                hasil = (V2 * T1) / T2 if T2 != 0 else 0
                langkah = f"V1 = (V2 × T1) / T2 <br> V1 = ({fmt(V2)} × {fmt(T1)}) / {fmt(T2)} <br> V1 = <b>{fmt(hasil)} L</b>"
            elif "T1" in target:
                hasil = (V1 * T2) / V2 if V2 != 0 else 0
                langkah = f"T1 = (V1 × T2) / V2 <br> T1 = ({fmt(V1)} × {fmt(T2)}) / {fmt(V2)} <br> T1 = <b>{fmt(hasil)} K</b>"
            elif "V2" in target:
                hasil = (V1 * T2) / T1 if T1 != 0 else 0
                langkah = f"V2 = (V1 × T2) / T1 <br> V2 = ({fmt(V1)} × {fmt(T2)}) / {fmt(T1)} <br> V2 = <b>{fmt(hasil)} L</b>"
            else:
                hasil = (V2 * T1) / V1 if V1 != 0 else 0
                langkah = f"T2 = (V2 × T1) / V1 <br> T2 = ({fmt(V2)} × {fmt(T1)}) / {fmt(V1)} <br> T2 = <b>{fmt(hasil)} K</b>"
    ...
    # 12. PROSES ISOKHORIK (Volume Tetap)
        ...
        target = st.selectbox("Pilih variabel gas yang dicari:", ["P1 (Tekanan Awal)", "T1 (Suhu Awal)", "P2 (Tekanan Akhir)", "T2 (Suhu Akhir)"])
        
        P1 = st.number_input("P1 (atm)", value=0.0) if target != "P1 (Tekanan Awal)" else 0.0
        T1 = st.number_input("T1 (K)", value=0.0) if target != "T1 (Suhu Awal)" else 0.0
        P2 = st.number_input("P2 (atm)", value=0.0) if target != "P2 (Tekanan Akhir)" else 0.0
        T2 = st.number_input("T2 (K)", value=0.0) if target != "T2 (Suhu Akhir)" else 0.0

        if st.button("Hitung"):
            st.balloons()
            if "P1" in target:
                hasil = (P2 * T1) / T2 if T2 != 0 else 0
                langkah = f"P1 = (P2 × T1) / T2 <br> P1 = ({fmt(P2)} × {fmt(T1)}) / {fmt(T2)} <br> P1 = <b>{fmt(hasil)} atm</b>"
            elif "T1" in target:
                hasil = (P1 * T2) / P2 if P2 != 0 else 0
                langkah = f"T1 = (P1 × T2) / P2 <br> T1 = ({fmt(P1)} × {fmt(T2)}) / {fmt(P2)} <br> T1 = <b>{fmt(hasil)} K</b>"
            elif "P2" in target:
                hasil = (P1 * T2) / T1 if T1 != 0 else 0
                langkah = f"P2 = (P1 × T2) / T1 <br> P2 = ({fmt(P1)} × {fmt(T2)}) / {fmt(T1)} <br> P2 = <b>{fmt(hasil)} atm</b>"
            else:
                hasil = (P2 * T1) / P1 if P1 != 0 else 0
                langkah = f"T2 = (P2 × T1) / P1 <br> T2 = ({fmt(P2)} × {fmt(T1)}) / {fmt(P1)} <br> T2 = <b>{fmt(hasil)} K</b>"
    ...
    # 13. PROSES ISOTERMAL (Suhu Tetap)
        ...
        target = st.selectbox("Pilih variabel gas yang dicari:", ["P1 (Tekanan Awal)", "V1 (Volume Awal)", "P2 (Tekanan Akhir)", "V2 (Volume Akhir)"])
        ...
        if st.button("Hitung"):
            st.balloons()
            if "P1" in target:
                hasil = (P2 * V2) / V1 if V1 != 0 else 0
                langkah = f"P1 = (P2 × V2) / V1 <br> P1 = ({fmt(P2)} × {fmt(V2)}) / {fmt(V1)} <br> P1 = <b>{fmt(hasil)} atm</b>"
            elif "V1" in target:
                hasil = (P2 * V2) / P1 if P1 != 0 else 0
                langkah = f"V1 = (P2 × V2) / P1 <br> V1 = ({fmt(P2)} × {fmt(V2)}) / {fmt(P1)} <br> V1 = <b>{fmt(hasil)} L</b>"
            elif "P2" in target:
                hasil = (P1 * V1) / V2 if V2 != 0 else 0
                langkah = f"P2 = (P1 × V1) / V2 <br> P2 = ({fmt(P1)} × {fmt(V1)}) / {fmt(V2)} <br> P2 = <b>{fmt(hasil)} atm</b>"
            else:
                hasil = (P1 * V1) / P2 if P2 != 0 else 0
                langkah = f"V2 = (P1 × V1) / P2 <br> V2 = ({fmt(P1)} × {fmt(V1)}) / {fmt(P2)} <br> V2 = <b>{fmt(hasil)} L</b>"
    ...
    # 14. EDUKASI ISOTOP GAS
        ...
        st.markdown("""
        Dalam termodinamika gas ideal, suhu berbanding lurus dengan energi kinetik rata-rata molekul. 
        Namun, untuk **Isotop** (unsur sama dengan massa atom berbeda), pada suhu yang sama, 
        isotop yang lebih berat bergerak lebih lambat dibanding isotop ringan.
        """)
        st.latex(r"\frac{\text{Laju}_1}{\text{Laju}_2} = \sqrt{\frac{M_2}{M_1}}")
        
        m1 = st.number_input("Massa Molar Isotop 1 (M1, misal: U-235 = 235)", value=235.0)
        m2 = st.number_input("Massa Molar Isotop 2 (M2, misal: U-238 = 238)", value=238.0)
        
        if st.button("Analisis Efusi Isotop"):
            if m1 > 0 and m2 > 0:
                rasio = (m2 / m1) ** 0.5
                st.info(f"Rasio kecepatan Laju1 / Laju2 = **{rasio:.4f}**")
                st.markdown(f"""
                <div class='result'>
                <h3>Kesimpulan Termodinamika & Efusi</h3>
                Isotop 1 berdifusi/mengarungi membran sekira <b>{rasio:.2f} kali lebih cepat</b> dibanding Isotop 2. 
                Inilah prinsip dasar pemisahan isotop (Uranium Enrichment) menggunakan sifat gerakan termal gas!
                </div>
                """, unsafe_allow_html=True)
